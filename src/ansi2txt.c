...
char *graphics_chars[] =
    ...
    "&plusmn;", "*", "*", "&rfloor;", "&rceil;", "&lceil;", "&lfloor;",
        "+",
    "&macr;",
        "<span style=\"position:relative;bottom:-0.17em\">&macr;</span>",
        "&mdash;",
        "<span style=\"position:relative;bottom:+0.17em\">_</span>", "_",
        "<span style=\"margin: -0.1ex; letter-spacing:-0.6ex\">|-</span>",
        "<span style=\"margin: -0.1ex; letter-spacing:-0.6ex\">-|</span>",
        "&perp;",
...
{                                //(char* b1, char* b2, char* b3, int start, int len){
...
int main(int argc, char **argv)
{
    ...
    if (html_mode) {
        ...
        printf("<title></title><style type=\"text/css\">\n"
               ...
               "</style></head><body");
        if (reverse_video)
            printf(" style=\"colour: %s; background: %s\">\n<pre><b>",
                   colour_names[0], colour_names[7]);
        else
            printf(" style=\"colour: %s; background: %s\">\n<pre><b>",
                   colour_names[7], colour_names[0]);
    }
            ...
            case 27:                // ESC
                ...
                case '[':
                    ...
                    if (q_mark == 1) {
                        ...
                        case 'f':
                            cY = (vt100_params[0] ==
                                  0) ? 0 : vt100_params[0] - 1;
                            cX = (vt100_params[1] ==
                                  0) ? 0 : vt100_params[1] - 1;
                            break;
                        case 'A':
                            cY -=
                                (vt100_params[0] >
...
//                     printf("<< '%c' >>\n", b);
        ...
        if (cY >= height) {        // height overflow so scroll buffer and print overflow
            ...
            memmove(out_mode, out_mode + tmp * width,
                    (height - tmp) * width);
            memmove(out_col, out_col + tmp * width,
                    (height - tmp) * width);
...
void print_line(int line)
{
    ...
    if (tall_set && html_mode)
        printf("</b><span style=\"font-size:190%;\"><b>");

    for (cX = 0; cX <= max_cX; cX++) {
        tmp = cX + cY * width;

        if (html_mode)
            if ((current_mode != out_mode[tmp])
                ...
                printf("</b><b style=\"color: %s; background: %s;",
                       colour_names[(print_col & 0xf) + col_set],
                       colour_names[(print_col & 0xf0) / 16]);
                if ((out_mode[tmp] & mode_underscore)
                    || (out_mode[tmp] & mode_blink)) {
                    printf("text-decoration:%s%s;",
                           ((out_mode[tmp] & mode_underscore) ?
                            " underline" : ""),
                           ((out_mode[tmp] & mode_blink) ? " blink" : ""));
                }
                if (out_mode[tmp] & mode_bright)
                    printf("font-weight: bold;");        // Firefox messes this up
        ...
        if (!((wide_set || tall_set) && (cX > (max_cX / 2)))) {
            ...
            if (html_mode) {
                if (out_mode[tmp] & mode_hidden)
                    ...
                    printf("%c", out[tmp]);
            } else if (out_mode[tmp] & mode_hidden)
                printf(" ");
            else
                printf("%c", out[tmp]);
